...
exports.createDataSimulator = createDataSimulator;
...
function wrapCoordinate(value, min, max) {
    const span = max - min;
    if (span <= 0)
        return value;
    let wrapped = value;
...
async function simulateVehicles(client) {
    ...
    return rows.map((row) => row.veh_id);
}
async function simulateRsus(client, simClock) {
    const { rows } = await client.query(`
    ...
    for (const row of rows) {
        ...
        const cpuCapacity = typeof row.cpu_capacity_mips === 'number' ? Number(row.cpu_capacity_mips) : 10000;
    ...
    return rows.map((row) => row.rsu_id);
}
function advanceTaskStatus(current) {
...
async function simulateTasks(client, simClock, vehicleIds, rsuIds) {
    const { rows } = await client.query(`
      SELECT
        assignment_id,
        task_id,
    ...
    const { rows: counts } = await client.query(`SELECT COUNT(*)::text AS count FROM edge_tasks WHERE status IN ('pending','assigned','executing')`);
    ...
    if (shouldSpawn && vehicleIds.length > 0 && rsuIds.length > 0) {
        ...
        const { rows: newTaskRows } = await client.query(`
      ...
      `, [
            ...
            takenVehicle,
            takenRsu,
            randomInt(400000000, 1600000000),
            randomBetween(10, 60),
...
async function simulateAlerts(client, simClock, vehicleIds, rsuIds) {
    if (Math.random() > 0.55) {
...
function createDataSimulator(options = {}) {
